...
% Inhibitory Ratio
[fh,ax,exponentialFit]=dendrite.l2vsl3vsl5.plotCorrelation(distance2soma,shaftRatio);
% plot exponential with offset
modelfun = @(b,x)(b(1)+b(2)*exp(b(3)*x));
distRange=linspace(0,500,500);
modelRatio=modelfun(exponentialFit.oneWithOff.Coefficients.Estimate',...
    distRange);
...
disp(['Single exponential fit Rsquared, Ratio: ',...
    num2str(exponentialFit.one.gof.rsquare)]);
disp(['Double exponential fit Rsquared, Ratio: ',...
    num2str(exponentialFit.two.gof.rsquare)]);

% shaft density
[fh,ax,exponentialFit]=dendrite.l2vsl3vsl5.plotCorrelation...
    (distance2soma,shaftDensity);
%plot(exp.two.f,'k');
legend('off')
xlabel([]);
ylabel([]);
...
disp(['Single exponential fit Rsquared, Shaft: ',...
    num2str(exponentialFit.one.gof.rsquare)]);
disp(['Double exponential fit Rsquared, Shaft: ',...
    num2str(exponentialFit.two.gof.rsquare)]);
...
disp(['Single exponential fit Rsquared, Spine: ',...
    num2str(exponentialFit.one.gof.rsquare)]);
disp(['Double exponential fit Rsquared, Spine: ',...
    num2str(exponentialFit.two.gof.rsquare)]);