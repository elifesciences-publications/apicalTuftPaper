...
% REARRANGEARRAYFORPLOT extracts variables from "results" with all the data 
% to pick specific "variables" for plotting. This plotting variables contains structure with
% each one having a cell array inside for the cell type
% Note: results fields should match layer Origin
...
for l = 1:length(layerOriginOrDataset)
    ...
    for i = 1:length(variables)
        % extract variables from the table components of curResults into
        if ~iscell(variables{i})
            ...
            % Note: Having a cell array component(2 cells) in the variables 
            % combines the two variables for correlation plotting
...
if aggregateDatasetResults
    % Go over each field and aggregate the data over different datasets. 
    % Only the celltype variation is present
