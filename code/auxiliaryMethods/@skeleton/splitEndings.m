...
nodesReachableEdges = cell(skel.numTrees,1);
...
for tree_index = 1:skel.numTrees
    ...
    if ~isempty(nodes)
        for i=1:size(nodes,1)
            ...
            nodesReachableEdges{tree_index}{i} = extractEdgeList(skel, tree_index,nodesReachable{tree_index}{i});
            if closeGap
...
for tree_index = 1:skel.numTrees
    ...
    if ~isempty(nodesR)
        for i=1:numel(nodesR)
            skelEnd = skelEnd.addTree('',skel.nodes{tree_index}(nodesR{i},:), nodesReachableEdges{tree_index}{i});
            toDel = vertcat(toDel,find(nodesRD{i}));
        end
    skelNew = skelNew.deleteNodes(tree_index,toDel, true);
    end
end
